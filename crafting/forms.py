...
class BaseQuoteForm:
    """Base form class with common functionality for all quote forms"""
    def __init__(self, *args, **kwargs):
...
class DigitizingQuoteForm(BaseQuoteForm, forms.ModelForm):
    file1 = forms.FileField(label='Upload Files (2 MB Max Size)', required=False)
    file2 = forms.FileField(label='', required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Row(
                Column('is_urgent', css_class='col-md-8'),
                Column(Submit('submit', 'Request Quote', css_class='btn btn-primary w-100'), css_class='col-md-2 text-end'),
                Column(Reset('reset', 'Reset', css_class='btn btn-primary w-100'), css_class='col-md-2 text-end'),
            ),
        )
    ...
    class Meta:
        ...
        labels = {
            ...
            'logo_placement': 'Logo Placement',
            'instructions': 'Special Instructions',
            'is_urgent': 'Is this quote urgent?',
        }
        widgets = {
            'instructions': forms.Textarea(attrs={'rows': 3})
        }
    ...
    def save(self, commit=True):
        ...
        if file1:
            DigitizingQuote_Files.objects.create(quote=quote, file=file1)
        if file2:
            DigitizingQuote_Files.objects.create(quote=quote, file=file2)

        return quote

class PatchQuoteForm(BaseQuoteForm, forms.ModelForm):
    file1 = forms.FileField(label='Upload Files (2 MB Max Size)', required=False)
    file2 = forms.FileField(label='', required=False)    
    ...
    def save(self, commit=True):
        ...
        if file1:
            PatchQuote_Files.objects.create(quote=quote, file=file1)
        if file2:
            PatchQuote_Files.objects.create(quote=quote, file=file2)

        return quote

class VectorQuoteForm(BaseQuoteForm, forms.ModelForm):
    file1 = forms.FileField(label='Upload Files (2 MB Max Size)', required=False)
    file2 = forms.FileField(label='', required=False)    
    ...
    def save(self, commit=True):
        ...
        if file1:
            VectorQuote_Files.objects.create(quote=quote, file=file1)
        if file2:
            VectorQuote_Files.objects.create(quote=quote, file=file2)
